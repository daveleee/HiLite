<?php
	...
	$sql1='update tbl_notice set notice_views=notice_views+1 where notice_no='.$notice_no;
...
<html>
	<head>
	    ...
	    <script>
	    	function go_back() {
	    		location.href="./php/admin_notice.php";
			}
			function del_notice() {
	    		location.href="./admin_notice_delete_db.php?notice_no=<?php echo $notice_no?>";
			}
	    </script>
	</head>
...
<?php
	$sql2='select * from tbl_notice, tbl_gallery where notice_no=gal_notice_no and notice_no='.$notice_no;
	$result2=mysqli_query($conn, $sql2);
	$row2=mysqli_fetch_assoc($result2);

	if(empty($row2['gal_imgpath']))		//이미지 경로 없으면 
	{
		$sql3='select * from tbl_notice where notice_no='.$notice_no;
		$result3=mysqli_query($conn, $sql3);
		$row3=mysqli_fetch_assoc($result3);
?>
    			<form name="member-form" action="admin_notice_update_db.php?notice_no=<?php echo $row3['notice_no']; ?>" method="post" class="admin-member-form">
    				<ul>
    					<li>
    						<label for="subject">제목</label>
    						<input type="text" class="form-control" name="ftitle" id="notice_subject" value="<?php echo $row3['notice_title']; ?>" />
    					</li>
    					<li>
    						<label for="content">내용</label>
    						<textarea class="form-control" name="ftext" rows="30"><?php echo $row3['notice_text']; ?></textarea>
    					</li>
    					<li>
    						<label for="image">이미지 경로</label>
    						<input type="text" class="form-control" name="fimgpath" id="notice_img" />
    					</li>
    				...
    				<nav class="btngroup">
    					...
    					<button type="button" class="btn btn-default" onclick="del_notice()">삭제처리</button>
...
<?php
	...
	else 	// 이미지 경로 있으면
	{
		...
		$sql4='select * from tbl_gallery where gal_notice_no='.$notice_no;
		$result4=mysqli_query($conn, $sql4);
		$row4=mysqli_fetch_assoc($result4);
?>
    			<form name="member-form" action="admin_notice_update_db2.php?notice_no=<?php echo $row3['notice_no']; ?>" method="post" class="admin-member-form">
    				<ul>
    					<li>
    						<label for="subject">제목</label>
    						<input type="text" class="form-control" name="ftitle" id="notice_subject" value="<?php echo $row3['notice_title']; ?>" />
    					...
    					<li>
    						<label for="image">이미지 경로</label>
    						<input type="text" name="fimgpath" class="form-control" id="notice_img" value="<?php echo $row4['gal_imgpath']; ?>"/>
    					</li>
