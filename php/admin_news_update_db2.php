<!-- 뉴스 내용 수정하는 페이지2 이미지가 기존에 존재할 경우-->
<?php
	session_start();
	$s=$_SESSION['user_id'];
	...
	require_once($_SERVER["DOCUMENT_ROOT"] . '/hilite/config.php');
		...
		$sql='select gal_imgpath from tbl_gallery where gal_news_no='.$news_no;
	...
	{
		$sql1='update tbl_news set news_title="'.$ftitle.'" where news_no="'.$news_no.'"';
		$result1=mysqli_query($conn, $sql1);

		$sql2='update tbl_news set news_text="'.$ftext.'" where news_no="'.$news_no.'"';
	...
	{
		$sql3='update tbl_news set news_title="'.$ftitle.'" where news_no="'.$news_no.'"';
		$result3=mysqli_query($conn, $sql3);

		$sql4='update tbl_news set news_text="'.$ftext.'" where news_no="'.$news_no.'"';
		$result4=mysqli_query($conn, $sql4);

		$sql5='UPDATE tbl_gallery SET gal_imgpath="'.$fimgpath.'" , gal_date=now() WHERE gal_news_no="'.$news_no.'" ';
