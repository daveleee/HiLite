...
<!-- 노티스인 데이터 푸싱 시작-->
<?php
	$sql2="select count(*) from tbl_notice, tbl_gallery where notice_no=gal_notice_no and notice_no=".$notice_no;
	$result2=mysqli_query($conn, $sql2);
	$row2=mysqli_fetch_assoc($result2);
	if($row2['count(*)']==0)
	{
		$sql3="select notice_views, notice_title, notice_id, notice_date, notice_text, notice_views, gal_imgpath from tbl_notice, tbl_gallery where notice_no=".$notice_no;
...
?>
	    	...
	    	<div class="single-title">
	    		<h1><?php echo $row3['notice_title']; ?></h1>
	    		<div class="clear"></div>
	    			<div class="single-info">
	    				<div class="single-data">	
	    					<?php echo $row3['notice_date']; ?>
						</div>
	    				<div class="single-tag">
	    					작성자:
	    					
	    						<?php echo $row3['notice_id']; ?>
	    					
	    					
	    					<span>조회수:<?php echo $row3['notice_views']; ?></span>
	    				</div>
	    			</div>
...
<?php
	...
	{
		$sql3="select notice_views, notice_title, notice_id, notice_date, notice_text, notice_views, gal_imgpath from tbl_notice, tbl_gallery where notice_no=gal_notice_no and notice_no=".$notice_no;
		$result3=mysqli_query($conn, $sql3);
