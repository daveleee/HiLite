...
echo $s." 님 환영합니다.";
    ...
    <?php
      ...
      if($page != 1) 
      { 
        $paging .= '<li class="page_start"><a href="./news.php?page=1">처음</a></li>';
      }
      //첫 섹션이 아니라면 이전 버튼을 생성
      if($currentSection != 1) 
      { 
        $paging .= '<li class="page_prev"><a href="./news.php?page=' . $prevPage . '">이전</a></li>';
      }
      ...
      for($i = $firstPage; $i <= $lastPage; $i++) 
      {
        ...
        {
          $paging .= '<li class="page_page"><a href="./news.php?page=' . $i . '">' . $i . '</a></li>';
        }
      }
      ...
      if($currentSection != $allSection) 
      { 
        $paging .= '<li class="page_next"><a href="./news.php?page=' . $nextPage . '">다음</a></li>';
      }
      
      //마지막 페이지가 아니라면 끝 버튼을 생성
      if($page != $allPage) 
      { 
        $paging .= '<li class="page_end"><a href="./news.php?page=' . $allPage . '">끝</a></li>';
      }
      $paging .= '</ul>';
      ...
      <div id="scbd" class="scbd co-red mo">
        ...
        <div id="list_board" class="list_board">
          ...
          <ul class="lst-board lst-body">
            <?php
              $sql='select * from tbl_news order by news_no desc';
              $result=mysqli_query($conn, $sql);

              while($row=mysqli_fetch_assoc($result_num))
              {
                $datetime=explode(' ', $row['news_date']);
                $date=$datetime[0];
                $time=$datetime[1];
                if($date==Date('Y-m-d'))
                {
            ...
            ?>
                    ...
                    <li class="clr">
                        <div class="td col_no">
                <?php echo $row['news_no']; ?>
              </div>
                        
              <div class="td col_subject" >
                <a href="./news_in.php?news_no=<?php echo $row['news_no']; ?>">
                ...
                <?php echo $row['news_id']; ?>
                        ...
                        <div class="td inf col_date"><?php echo $row['news_date']; ?></div>
                        <div class="td inf col_hit"><span class="txt">조회수 :</span>
                <?php echo $row['news_views']; ?>
