...
echo $s." 님 환영합니다.";
    ...
    require_once($_SERVER["DOCUMENT_ROOT"] . '/hilite/config.php');
      ...
      $sql = 'select count(*) as cnt from tbl_news order by news_no desc';
    ...
    <?php
      ...
      if($page != 1) 
      { 
        $paging .= '<li class="page_start"><a href="./admin_news.php?page=1">처음</a></li>';
      }
      //첫 섹션이 아니라면 이전 버튼을 생성
      if($currentSection != 1) 
      { 
        $paging .= '<li class="page_prev"><a href="./admin_news.php?page=' . $prevPage . '">이전</a></li>';
      }
      ...
      for($i = $firstPage; $i <= $lastPage; $i++) 
      {
        ...
        {
          $paging .= '<li class="page_page"><a href="./admin_news.php?page=' . $i . '">' . $i . '</a></li>';
        }
      }
      ...
      if($currentSection != $allSection) 
      { 
        $paging .= '<li class="page_next"><a href="./admin_news.php?page=' . $nextPage . '">다음</a></li>';
      }
      
      //마지막 페이지가 아니라면 끝 버튼을 생성
      if($page != $allPage) 
      { 
        $paging .= '<li class="page_end"><a href="./admin_news.php?page=' . $allPage . '">끝</a></li>';
      }
      $paging .= '</ul>';
      ...
      $sql = 'select * from tbl_news order by news_no desc' . $sqlLimit; //원하는 개수만큼 가져온다. (0번째부터 20번째까지
...
<html>
	<head>
	    ...
	    <script type="text/javascript">
	    	function go_write() {
	    		location.href="./admin_news_write.php";
	    	}
	    </script>
	</head>
...
<?php
	$sql1='select * from tbl_news order by news_no desc';
...
?>
	    					...
	    					<div class="td col_subject">
	    						<div style="padding-left:0px;">
	    							<a href="./admin_news_in.php?news_no=<?php echo $row1['news_no']; ?>">
	    								<span>
<?php
	echo $row1['news_title'];
?>
	    								</span>
	    							</a>
	    						</div>
