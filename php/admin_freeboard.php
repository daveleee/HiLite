...
echo $s." 님 환영합니다.";
    ...
    require_once($_SERVER["DOCUMENT_ROOT"] . '/hilite/config.php');
      ...
      $sql = 'select count(*) as cnt from tbl_freeboard order by fb_no desc';
    ...
    <?php
      ...
      if($page != 1) 
      { 
        $paging .= '<li class="page_start"><a href="./admin_freeboard.php?page=1">처음</a></li>';
      }
      //첫 섹션이 아니라면 이전 버튼을 생성
      if($currentSection != 1) 
      { 
        $paging .= '<li class="page_prev"><a href="./admin_freeboard.php?page=' . $prevPage . '">이전</a></li>';
      }
      ...
      for($i = $firstPage; $i <= $lastPage; $i++) 
      {
        ...
        {
          $paging .= '<li class="page_page"><a href="./admin_freeboard.php?page=' . $i . '">' . $i . '</a></li>';
        }
      }
      ...
      if($currentSection != $allSection) 
      { 
        $paging .= '<li class="page_next"><a href="./admin_freeboard.php?page=' . $nextPage . '">다음</a></li>';
      }
      
      //마지막 페이지가 아니라면 끝 버튼을 생성
      if($page != $allPage) 
      { 
        $paging .= '<li class="page_end"><a href="./admin_freeboard.php?page=' . $allPage . '">끝</a></li>';
      }
      $paging .= '</ul>';
      ...
      $sql = 'select * from tbl_freeboard order by fb_no desc' . $sqlLimit; //원하는 개수만큼 가져온다. (0번째부터 20번째까지
  ...
  <head>
    ...
    <script src="../js/ie-emulation-modes-warning.js"></script>
      ...
      <script type="text/javascript">
	    	function go_write() {
	    		location.href="./admin_freeboard_write.php";
	    	};
      ...
      <div id="scbd" class="scbd co-red mo">
        ...
        <div id="list_board" class="list_board">
          ...
          <ul class="lst-board lst-body">
            <?php
              $sql='select * from tbl_freeboard order by fb_no desc';
              $result=mysqli_query($conn, $sql);

              while($row=mysqli_fetch_assoc($result_num))
              {
                $datetime=explode(' ', $row['fb_date']);
                $date=$datetime[0];
                $time=$datetime[1];
                if($date==Date('Y-m-d'))
                {
            ...
            ?>
              ...
              <div class="td col_subject" >
                <a href="./admin_freeboard_in.php?fb_no=<?php echo $row['fb_no']?>">
                <?php echo $row['fb_title']?> </a>
                ...
                <?php echo $row['fb_id']?>
    					...
    					<div class="td inf col_date"><?php echo $row['fb_date']?></div>
    					<div class="td inf col_hit"><span class="txt">조회수 :</span>
                <?php echo $row['fb_views']?>
