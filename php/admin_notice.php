...
echo $s." 님 환영합니다.";
    ...
    <?php
      ...
      if($page != 1) 
      { 
        $paging .= '<li class="page_start"><a href="./admin_notice.php?page=1">처음</a></li>';
      }
      //첫 섹션이 아니라면 이전 버튼을 생성
      if($currentSection != 1) 
      { 
        $paging .= '<li class="page_prev"><a href="./admin_notice.php?page=' . $prevPage . '">이전</a></li>';
      }
      ...
      for($i = $firstPage; $i <= $lastPage; $i++) 
      {
        ...
        {
          $paging .= '<li class="page_page"><a href="./admin_notice.php?page=' . $i . '">' . $i . '</a></li>';
        }
      }
      ...
      if($currentSection != $allSection) 
      { 
        $paging .= '<li class="page_next"><a href="./admin_notice.php?page=' . $nextPage . '">다음</a></li>';
      }
      
      //마지막 페이지가 아니라면 끝 버튼을 생성
      if($page != $allPage) 
      { 
        $paging .= '<li class="page_end"><a href="./admin_notice.php?page=' . $allPage . '">끝</a></li>';
      }
      $paging .= '</ul>';
      ...
      $sql2 = 'select * from tbl_notice order by notice_no desc' . $sqlLimit; //원하는 개수만큼 가져온다. (0번째부터 20번째까지
      $result_num = mysqli_query($conn, $sql2);
...
<html>
	<head>
	    ...
	    <script type="text/javascript">
	    	function go_write() {
	    		location.href="./admin_notice_write.php";
	    	}
	    </script>
	</head>
    		...
    		<div id="lay-hd" class="lay-hd">
    			<div class="hgroup">
    				...
    				<ul>
    					...
    					<li>
    						<button type="button" class="btn btn-link" onClick="go_write()" target="self" style="color:#ffffff; width:100%; height: 100%;">
...
<?php
	$sql1='select * from tbl_notice order by notice_no desc';
	$result1=mysqli_query($conn, $sql1);
	while($row1=mysqli_fetch_assoc($result_num))
	{
?>
    				<li class="clr">
    					<div class="td col_no">
...
?>
	    					...
	    					<div class="td col_subject">
	    						<div style="padding-left:0px;">
	    							<a href="./admin_notice_in.php?notice_no=<?php echo $row1['notice_no']; ?>">
	    								<span>
<?php
