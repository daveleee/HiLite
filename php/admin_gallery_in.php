...
<html>
	<head>
	    ...
	    <script>
	    	function go_back() {
	    		location.href="./admin_gallery.php";
			}
			
			function del_gallery()
			{
				location.href="./admin_gallery_delete_db.php?mus_ename=<?php echo $mus_ename; ?>&gal_no=<?php echo $gal_no; ?>";
			}
	    </script>
	</head>
    			...
    			<form name="member-form" class="admin-member-form">
    				<ul>
    					<li>
    						...
    						<input type="text" value="<?php echo $mus_ename; ?>" class="form-control" readonly="readonly" name="gallery_name" id="gallery_name" />
    					</li>
<?php
	$sql1='select * from tbl_gallery where gal_no='.$gal_no;
	$result1=mysqli_query($conn, $sql1);
	$row1=mysqli_fetch_assoc($result1)
?>
    					<li>
    						<label for="image">이미지 경로</label>
    						<input type="text" value="<?php echo $row1['gal_imgpath']; ?>"class="form-control" name="gallery_img" id="gallery_img" />
    					</li>
    					
    				</ul> 
    				
    				<nav class="btngroup">
    					<button type="button" class="btn btn-default" onclick="del_gallery()">삭제처리</button>
